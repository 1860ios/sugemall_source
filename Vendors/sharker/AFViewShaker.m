...
static NSTimeInterval const kAFViewShakerDefaultDuration = 0.3;
...
- (void)shakeWithDuration:(NSTimeInterval)duration completion:(void (^)())completion {
    ...
    for (UIView * view in self.views) {
        [self addShakeAnimationForView1:view withDuration:duration];
    }
}
...
////////////////////
- (void)addShakeAnimationForView1:(UIView *)view withDuration:(NSTimeInterval)duration {
    CAKeyframeAnimation * animation = [CAKeyframeAnimation animationWithKeyPath:@"transform.translation.x"];
    CGFloat currentTx = view.transform.tx;
    ...
    animation.duration = duration;
    animation.values = @[ @(currentTx), @(currentTx + 10), @(currentTx-8), @(currentTx + 8), @(currentTx -5), @(currentTx + 5), @(currentTx) ];
    animation.keyTimes = @[ @(0), @(0.125), @(0.525), @(0.6), @(0.75), @(0.875), @(1) ];
