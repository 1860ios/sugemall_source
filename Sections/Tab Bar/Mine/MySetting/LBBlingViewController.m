...
@interface LBBlingViewController ()<UITextFieldDelegate>
{
    UITextField *moblieT;
    UITextField *vCodeT;
...
@implementation LBBlingViewController
...
- (void)setUpView3
{
    ...
    [nextButton setTitle:@"更换手机号" forState:0];
...
- (void)doneBlindMobile
{
    if (moblieT.text.length == 0||vCodeT.text.length  == 0 ) {
        [[[UIAlertView alloc]initWithTitle:@"错误" message:@"请填写完整信息!" delegate:self cancelButtonTitle:@"确定" otherButtonTitles: nil]show];
            ...
            NSString *key  =[LBUserInfo sharedUserSingleton].userinfo_key;
            AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
            NSDictionary *parameters = @{@"key":key,@"vcode":vCodeT.text,@"mobile":moblieT.text};
            manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"text/html",@"application/json",@"text/json",nil];
            
            [manager POST:SUGE_BLING_MOB parameters:parameters success:^(AFHTTPRequestOperation *operation,id responObject){
                
                NSLog(@"绑定新手机号:responObject:%@",responObject);
                ...
                if ([result isEqualToString:@"msg"]) {
                    ...
                    [self.navigationController popToViewController:[LBSafeViewController new] animated:YES];
                }else{
