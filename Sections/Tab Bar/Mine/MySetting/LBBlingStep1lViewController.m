...
@interface LBBlingStep1lViewController ()<UITextFieldDelegate>
{
    UITextField *moblieT;
    UITextField *vCodeT;
...
@implementation LBBlingStep1lViewController
@synthesize isNewEmail;
...
- (void)setUpView4
{
 ...
 //   moblieT.placeholder = @"请输入原始邮箱号";
   moblieT.text=_agoEmailString;
    moblieT.enabled=NO;
...
- (void)getVercode
{
    ...
    [manager POST:SUGE_SEND_EMAIL_VCODE parameters:parameters success:^(AFHTTPRequestOperation *operation,id responObject){
...
- (void)checkBlindMobile
{
    if (moblieT.text.length == 0||vCodeT.text.length  == 0) {
        ...
        NSString *key  =[LBUserInfo sharedUserSingleton].userinfo_key;
        AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
        NSDictionary *parameters = @{@"key":key,@"vcode":vCodeT.text,@"email":moblieT.text,@"type":@"email"};
        manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"text/html",@"application/json",@"text/json",nil];
        
        [manager POST:SUGE_CHECK_EMAIL_CODE parameters:parameters success:^(AFHTTPRequestOperation *operation,id responObject){
            
            NSLog(@"验证邮箱:responObject:%@",responObject);
//            NSString *success = ;
            if ([responObject[@"datas"] isEqual:@"1"]) {
                [TSMessage showNotificationWithTitle:@"提示" subtitle:@"验证邮箱成功" type:TSMessageNotificationTypeSuccess];
...
- (void)senderEmailCode:(UIButton *)btn
{
    ...
    [manager POST:SUGE_BLING_EMAIL parameters:parameters success:^(AFHTTPRequestOperation *operation,id responObject){
        ...
        NSLog(@"绑定新邮箱responObject:%@,%@",responObject,responObject);
